...
| 要素 | 説明 |
|------|------|
| サムネイル | クリックで YouTube 動画ページを開く（YouTube へ遷移するのはサムネイル部分のみ） |
| 再生時間 | サムネイル右下にオーバーレイ表示 |
| 再生数 | 現在の再生回数（切り捨て表示）。**クリックで推移グラフを開閉** |
| 勢い | 再生数の右側に「+XX万/日」形式で表示。**クリックで推移グラフを開閉** |
...
**再生数または勢いの数字をクリック**すると、カード内に直近30日の推移グラフが開きます（もう一度クリックで閉じる）。複数のカードを同時に開くこともできます。

| 要素 | 説明 |
|------|------|
| 青の線（左軸） | 再生数の推移（累積） |
| 緑の線（右軸） | 勢い＝再生数の増加ペース（万/日、24時間移動平均） |
| オレンジの帯＋破線 | 予測に使う直近72時間の区間と、その最小二乗近似直線 |
| 終点のラベル | 最新データの取得日時 |

グラフ上にマウスを乗せると、その時点の日付・再生数・勢いが上部に表示されます。

> データは直近7日が30分間隔、それ以前は2時間間隔で、最大30日分を保持しています。
...
> **推移グラフについて**：個人モードは取得データをブラウザの localStorage に保存する都合上、長期間ぶんの保持が難しく、再生数の推移グラフは**直近約7日分の表示**にとどまります（公開モードのような30日表示には対応していません）。
