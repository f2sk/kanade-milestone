...
**[ツールを開く →](https://f2sk.github.io/kanade-milestone/)**
...
- 直近72時間の再生数推移から ETA（予測到達日時）を自動算出
- 他チャンネル動画も URL 追加で一緒に管理（コラボ・参加楽曲など）
- ★マーク・フィルタ・ソートで見たい動画を絞り込み
- 節目達成時にブラウザ通知（★マーク済み動画）
- JSON でデータのエクスポート/インポート（端末間移行可）
- データはブラウザの localStorage にのみ保存（外部サーバーなし）
